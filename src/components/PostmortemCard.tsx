...
export function PostmortemCard({ postmortem }: Props) {
  return (
    <div className="border-l border-zinc-200 pl-4 py-1 dark:border-zinc-800">
      <div className="mb-1.5 flex items-baseline justify-between gap-4">
        <Link
          href={`/postmortems/${postmortem.slug}`}
          className="text-sm font-medium text-zinc-700 hover:text-zinc-950 dark:text-zinc-300 dark:hover:text-zinc-100"
        >
          {postmortem.title}
        </Link>
        <span className="shrink-0 text-xs text-zinc-500 dark:text-zinc-600">{postmortem.duration}</span>
      </div>

      <p className="mb-2 text-xs leading-relaxed text-zinc-600 dark:text-zinc-500">{postmortem.summary}</p>
